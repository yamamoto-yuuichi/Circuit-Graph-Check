...
サンプルとして扱う回路は、次の自己保持回路です。以降の説明は、この回路を例に進めます。

![自己保持回路_正常](回路図/自己保持回路_正常.png)

回路の構成は次のとおりです。

- **1段目（コイルの回路）**：起動ボタン `BS1_a` を押すとリレーコイル `R` がONになります。`R` がONになると、それに連動するリレーRの接点(1) `R_a1` もONになり、起動ボタンを離してもコイルがONのまま保たれます。停止ボタン `BS2_b` を押すと回路が切れ、コイルがOFFになります。
- **2段目（確認ランプの回路）**：リレーコイル `R` に連動するリレーRの接点(2) `R_a2` によって、運転確認ランプ `L` が点灯します。
...
- `BS1_a` は、押していないときに開いているa接点なので `a_contact` とします。
- `BS2_b` は、押していないときに閉じているb接点なので `b_contact` とします。
...
| 5行目 | `W1` | `R_a1` | `wire` | W1からリレーRの接点(1)へ |
| 6行目 | `R_a1` | `W2` | `wire` | リレーRの接点(1)からW2へ |
| 7行目 | `W2` | `BS2_b` | `wire` | W2から停止ボタンへ |
| 8行目 | `BS2_b` | `W3` | `wire` | 停止ボタンからW3へ |
| 9行目 | `W3` | `R` | `wire` | W3からリレーコイルへ |
| 10行目 | `R` | `W4` | `wire` | リレーコイルからW4へ |
| 11行目 | `W1` | `R_a2` | `wire` | W1からリレーRの接点(2)へ |
| 12行目 | `R_a2` | `W5` | `wire` | リレーRの接点(2)からW5へ |
| 13行目 | `W5` | `L` | `wire` | W5から運転確認ランプへ |
| 14行目 | `L` | `W4` | `wire` | 運転確認ランプからW4へ |
...
- **3〜6行目**が、起動ボタン `BS1_a` とリレーRの接点(1) `R_a1` の並列です。**どちらも `W1` と `W2` の間に置かれている**ため、これが並列（OR）になります。
- **7〜8行目**で、並列の合流点 `W2` から停止ボタン `BS2_b` を通り、`W3` に至ります。ここは直列（AND）です。
- **9〜10行目**で、リレーコイル `R` を `W3` と `W4` の間、つまりN母線側に置いています。
- **11〜14行目**が、運転確認ランプの回路です。`W1` からリレーRの接点(2) `R_a2` を通り、`W5` を経て運転確認ランプ `L` に至ります。
...
| （行） | 始点 | 終点 | type | この行が表しているもの |
| --- | --- | --- | --- | --- |
| 16行目 | `R` | `R_a1` | `link` | コイルRがONになるとリレーRの接点(1)も動く |
| 17行目 | `R` | `R_a2` | `link` | コイルRがONになるとリレーRの接点(2)も動く |
...
| --- | --- |
| `条件式に種類が含まれる` / `条件式に種類が含まれない` | 書いた値を、ノードリストの `kind` 列と照合します。その種類にあてはまる部品名をすべて集めたうえで、それらが条件式の中にあるかどうかを調べます。例：`b_contact` と書くと、`kind` が `b_contact` の部品が対象になります。 |
...
必要に応じて、`使い方` という名前のシートをもう1つ作り、編集するときの注意事項を書いておきます。**このシートはコードが読み込まないため、書式は自由です。
...
2. 本ノートブックをColabで開く。
3. Google Driveをマウントする。Colabの左メニューにあるフォルダのアイコンから「ドライブをマウント」を選ぶか、from google.colab import drive / drive.mount('/content/drive') を実行してください（このコードはツール本体には含まれていません）。
...
停止ボタン `BS2_b` とリレーRの接点(1) `R_a1` が、どちらも正しく配線されている回路です。「2. サンプル回路」の図と同じ構成です。
...
コイル `R` の条件式には、停止用のb接点 `BS2_b` と、起動ボタン `BS1_a` と並列に置かれたリレーRの接点(1) `R_a1` の両方が現れています。4つのルールすべてを満たすため、問題は報告されません。図では、すべての部品が水色のまま表示されます。

### 6.2. 回路構成部品リスト_自己保持回路_異常_停止手段があるか.xlsx

正常な回路から、**停止ボタン `BS2_b` を削除**した回路です。

**この回路の回路図**（赤い×印が、正常な回路から削除した箇所です）

![自己保持回路_異常_停止手段があるか](回路図/自己保持回路_異常_停止手段があるか.png)
...
### 6.3. 回路構成部品リスト_自己保持回路_異常_自己保持しているか.xlsx

正常な回路から、**リレーRの接点(1) `R_a1` を削除**した回路です。

**この回路の回路図**（赤い×印が、正常な回路から削除した箇所です）

![自己保持回路_異常_自己保持しているか](回路図/自己保持回路_異常_自己保持しているか.png)
...
安全ルールによる検査結果
  適用するルール: 4件
  --- 判定 ---
    [注意] リレーコイル `R` に自己保持用の接点が見当たりません。起動ボタンと並列に、リレーコイル `R` 自身の接点が入っていない可能性があります（連動する接点: R_a2）。
```

条件式が `(BS1_a AND BS2_b)` となり、**ORが1つも現れていません**。起動ボタンと並列になる接点が無いためです。連動エッジも1本（`R → R_a2`）に減っています。

「自己保持しているか」のルールが、リレーコイル `R` に連動する接点（`R_a2`）はあるものの、それが条件式の並列位置に無いことを検出し、`[注意]` として報告しています。**起動ボタンを押している間しか動かない回路**になっており、押しボタンを離すと止まってしまいます。
